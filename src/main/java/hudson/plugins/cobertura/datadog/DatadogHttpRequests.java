...
public class DatadogHttpRequests {
   ...
   */
  public static void sendEvent(DatadogEvent evt, final Secret apiKey,final String targetMetricURL) {
    logger.fine("Sending event");
    try {
      DatadogHttpRequests.post(evt.createPayload(), DatadogUtilities.METRIC, apiKey, targetMetricURL);
    } catch (Exception e) {
  ...
  public static Boolean post(final JSONObject payload, final String type, final Secret apiKey, final String targetMetricURL ) throws IOException {
   ...
   // String targetMetricURL= "https://api.datadoghq.com/api/";
    ...
    try {
      ...
      conn = DatadogHttpRequests.getHttpURLConnection(new URL(targetMetricURL + type + urlParameters));
      ...
      if (conn.getResponseCode() == DatadogUtilities.HTTP_FORBIDDEN) {
